...
@section('content')
  <div class="page-content">
    ...
    @include('backend.section.breadcrumb', ['title' => 'SubCategory', 'sub_title' => 'Edit-SubCategory'])
        ...
        <div class="card-body p-4">
          ...
          <form class="row g-3" method="post" action="{{ route('admin.subcategory.update', $sub_category->id) }}">
            ...
            <div class="col-md-6">
              ...
              <select class="form-select" name="category_id" id="single-select-field" data-placeholder="Choose one thing">
                <option></option>
                @foreach ($all_categories as $item)
                  <option value="{{ $item->id }}" {{$item->id == $sub_category->category_id ? 'selected' : '' }}>
                    {{ $item->name }}
                  </option>
                @endforeach
              </select>
