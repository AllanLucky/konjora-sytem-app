            ...
            @foreach ($categories as $index => $item)
                <li class="nav-item">
                    <a class="nav-link {{ $index == 0 ? 'active' : '' }}" id="{{ $item->slug }}-tab" data-toggle="tab"
                        href="#{{ $item->slug }}" role="tab" aria-controls="{{ $item->slug }}" aria-selected="true">
                        {{ $item->name }}
                    </a>
                </li>
            @endforeach
                                            ...
                                            <div class="course-badge-labels">
                                                <div class="course-badge">
                                                    @if ($course->bestseller == 1)
                                                        BestSeller
                                                    @elseif($course->featured == 1)
                                                        Featured
                                                    @elseif($course->highestrated == 1)
                                                        HighestRated
                                                    @else
                                                        &nbsp;
                                                    @endif
                                                </div>
                                                <div class="course-badge-danger">
                                                    -{{ round((($course->selling_price - $course->discount_price) / $course->selling_price) * 100) }}%
                                                </div>
                                            </div>
                                        </div><!-- end card-image -->

                                        <!-- Course Info Section (below image) -->
                                        <div class="card-body">
                                            <h6 class="fs-14 mb-3">
                                                {{ $course->label }}
                                            </h6>
